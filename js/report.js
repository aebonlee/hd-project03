...
(function (global) {
   ...
   * @param {File} file
   * @param {function(Error|null, {vendors:number, items:number}=)} done
   */
  function importMasterData(db, file, done) {
    var XLSX = global.XLSX;
          ...
          var vendors = vRows
            ...
            .map(function (r) {
              return {
                ...
                phone: String(r[3] || ''),
                email: String(r[4] || ''),
                password: String(r[5] || '1234')
              };
            });
          if (vendors.length > 0) { db.vendors = vendors; counts.vendors = vendors.length; }
        }
        if (wsItems) {
